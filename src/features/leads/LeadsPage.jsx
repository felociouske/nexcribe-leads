  ...
  return (
    <button
      ...
      className={`flex flex-col items-center rounded-xl py-3 px-2 transition-all border-2 ${
        active ? 'border-teal-500 bg-teal-50' : 'border-transparent bg-navy-100 hover:bg-navy-200'
      }`}
    >
      <span className={`font-display font-bold text-base ${active ? 'text-teal-700' : 'text-navy-700'}`}>
        C{level}
      </span>
      <span className={`text-xs mt-0.5 ${active ? 'text-teal-500' : 'text-navy-400'}`}>{count}</span>
    </button>
  )
}
...
function MembersPanel({ level, onClose }) {
  const { data, isLoading } = useQuery({
    queryKey: ['ref-downline-members', level],
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-5 py-4 border-b border-navy-100">
        <div>
          <h2 className="font-display font-bold text-navy-900">C{level} Members</h2>
          <p className="text-navy-500 text-xs mt-0.5">{data?.count ?? '...'} members</p>
        </div>
      ...
      <div className="flex gap-1.5 px-5 py-3 border-b border-navy-100 overflow-x-auto">
        {[1, 2, 3, 4].map(l => (
          <button key={l} onClick={() => { onClose(); setTimeout(() => onClose(l), 10) }}
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-display font-semibold transition-all ${
              l === level ? 'bg-teal-600 text-white' : 'bg-navy-100 text-navy-600 hover:bg-navy-200'
            }`}>
            C{l}
          </button>
        ))}
          ...
          members.map((m, i) => (
            <motion.div key={m.id}
              ...
              <div className="flex-1 min-w-0">
                <p className="font-display font-semibold text-navy-800 text-sm">{m.name || m.username}</p>
...
export default function LeadsPage() {
  ...
  return (
    <div className="page">
      <div className="mb-6">
        <h1 className="section-title">Leads &amp; Referrals</h1>
        <p className="section-subtitle">Track your network and commissions across 4 levels.</p>
      </div>

      {/* Referral link card */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl bg-gradient-to-br from-navy-900 via-navy-800 to-teal-900 text-white p-6 mb-6">
        <p className="text-white/70 text-xs font-display font-semibold uppercase tracking-widest mb-3">
          Your Referral Link
        </p>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={copyLink}
            className="bg-white text-teal-700 px-4 py-2 rounded-xl text-sm font-display font-semibold hover:bg-teal-50 transition-colors flex-shrink-0">
            Copy
          </button>
        </div>
        <div className="flex gap-6 mt-4">
          <div>
            <p className="font-display font-extrabold text-2xl">{referral?.total_referrals || 0}</p>
            <p className="text-white/60 text-xs">Direct referrals</p>
          </div>
          <div>
            <p className="font-display font-extrabold text-2xl">{totalDown}</p>
            <p className="text-white/60 text-xs">Total downline</p>
          </div>
          <div>
            <p className="font-display font-extrabold text-2xl">{fmtUSD(earnings?.yields_wallet_balance)}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-bold text-navy-900">Your Network</h2>
              <p className="text-xs text-navy-400">Tap a level to see members</p>
            </div>
            <div className="grid grid-cols-4 gap-3">
              {[1, 2, 3, 4].map(lvl => (
                <LevelBadge key={lvl} level={lvl} active={selectedLevel === lvl}
          ...
          {/* Earnings by level */}
          <div className="card p-6">
            <h2 className="font-display font-bold text-navy-900 mb-4">Earnings by Level</h2>
                ...
                return (
                  <div key={key} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 bg-navy-100 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-teal-600 to-teal-400 rounded-full transition-all duration-500"
                        style={{ width: `${Math.min((thisUsd / Math.max(totalUsd, 1)) * 100, 100)}%` }} />
                    </div>
                    <span className="w-20 text-right font-display font-semibold text-sm text-navy-700">
            ...
            <div className="mt-4 pt-4 border-t border-navy-100 flex items-center justify-between">
              <p className="font-display font-bold text-navy-700">Total earned</p>
              <p className="font-display font-extrabold text-xl text-teal-600">{fmtUSD(earnings?.total_earned_usd)}</p>
            </div>
          </div>

          {/* How it works */}
          <div className="card p-6 bg-navy-50/50">
            <h2 className="font-display font-bold text-navy-900 mb-3">How it works</h2>
            <ol className="space-y-2">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-sm text-navy-600">
                  <span className="w-5 h-5 rounded-full bg-teal-600 text-white text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">
                    {i + 1}
                  </span>
                  {step}
                </li>
        ...
        <div className="card overflow-hidden">
          {commissions.length === 0 ? (
            ...
            <table className="tbl">
              <thead>
                <tr>
                  <th>Code</th><th>From</th><th>Plan</th>
                  <th>Level</th><th className="text-right">Amount</th><th className="text-right">Date</th>
                </tr>
              </thead>
              <tbody>
                {commissions.map(c => (
                  <tr key={c.id}>
                    ...
                    <td className="font-medium">{c.from_user_username}</td>
                    <td>{c.plan_category} {c.plan_name}</td>
                    <td><span className="badge-navy">C{c.level_depth}</span></td>
                    <td className="text-right font-display font-bold text-teal-600">+{fmtUSD(c.amount_usd)}</td>
        ...
        {selectedLevel !== null && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/30 z-40" onClick={() => setSelectedLevel(null)} />
            <MembersPanel
              level={selectedLevel}
              onClose={(newLevel) => {
                if (typeof newLevel === 'number') setSelectedLevel(newLevel)
